...
def clean_all_labels(stage_name: str = "下面及捞面"):
    """
    删除所有标注文件（.txt文件，除了classes.txt）
    ...
    print("清理所有旧的标注文件")
    print("="*60)
    print("类别已更新为4个：noodle_rope, hand, tools_noodle, soup_noodle")
    ...
    print("  0: noodle_rope")
    print("  1: hand")
    print("  2: tools_noodle")
    print("  3: soup_noodle")
    print("\n下一步：")
    print("1. 使用新的4类别定义重新标注")
    print("2. 注意区分 soup_noodle（汤中面条）和 noodle_rope（其他情况下的面条）")
    print("="*60)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='清理所有旧的标注文件')
    parser.add_argument(
        '--confirm',
        action='store_true',
    ...
    if args.confirm:
        ...
        print("如果确定要删除，请运行: python scripts/clean_all_boiling_scooping_labels.py --confirm")
        print("\n当前状态下将要删除的文件：")
